...
// Mastercards — memory matching with themed card sets.
...
function flip(idx: number) {
  ...
  if (first === null) {
    ...
    if (cards[first].symbol === cards[second].symbol) {
      ...
      if (remaining === 0) {
        best = setHighScore("mastercards", moves, true);
